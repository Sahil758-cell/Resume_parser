...
def load_job_knowledge_base(file_path="job_knowledge_base.csv"):
    try:
        ...
        print(f"Error loading job knowledge base: {e}")
        return pd.DataFrame(columns=["job_title", "keywords", "recommended_courses", "skills"])

# Load job knowledge base into DataFrame
def predict_jobs(resume_text, top_n=5):
    # Normalize text
    resume_text = resume_text.lower()
    
    # Load job knowledge base
    df = load_job_knowledge_base()
    
    # Extract job titles
    job_titles = df["job_title"].tolist()
    
    # Create description texts for each job (combining all fields)
    job_descriptions = []
    for _, row in df.iterrows():
        desc = f"{row['job_title']} {row.get('keywords', '')} {row.get('skills', '')}"
        job_descriptions.append(desc.lower())
    
    # Initialize scores
    semantic_scores = [0.0] * len(job_titles)
    token_scores = [0.0] * len(job_titles)
    
    # Step 1: Attempt semantic search with SentenceTransformer if available
    semantic_model = None
    try:
        ...
        for i, desc in enumerate(job_descriptions):
            if desc:
                desc_embedding = semantic_model.encode(desc)
    ...
    # Step 2: Calculate token-based similarity (Jaccard)
    ...
    for i, row in df.iterrows():
        ...
        if 'ios' in job_title_lower:
            domain_specific_terms = ['swift', 'xcode', 'ios', 'objective-c', 'swiftui', 'mobile development', 'app development']
        ...
        elif 'finance analyst' in job_title_lower or 'financial analyst' in job_title_lower:
            domain_specific_terms = ['finance', 'analytics', 'excel', 'tally', 'financial modeling', 'accounting', 'banking', 'budgeting', 'forecasting']
        ...
        # --- SPECIFIC HANDLING FOR FINANCE CANDIDATES ---
        # If resume has finance keywords, heavily penalize tech roles
        if 'finance and analytics' in resume_text.lower() or 'mba: finance' in resume_text.lower():
            if 'ios developer' in job_title_lower or 'android developer' in job_title_lower:
                final_score = final_score * 0.3  # 70% penalty for tech roles for finance candidates
...
def get_recommended_courses(job_title, df=None):
    """Get recommended courses for a specific job title"""
    if df is None:
        df = load_job_knowledge_base()
    
    job_row = df[df["job_title"] == job_title]
    
    if not job_row.empty and "recommended_courses" in job_row.columns:
        courses_str = job_row["recommended_courses"].iloc[0]
        if courses_str:
            return [course.strip() for course in courses_str.split(";") if course.strip()]
    return []
